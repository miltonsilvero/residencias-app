...
export default function DashboardPage() {
  ...
  // Años y meses disponibles para el selector, en base a los periodos que
  // ya existen (para no poder navegar a un mes que todavía no se registró).
  const years = Array.from(new Set(periods.map((p) => p.month.slice(0, 4)))).sort(
  ...
  const currentMonthSelection = monthsForYear.some((p) => p.month === selectedMonthKey)
    ? selectedMonthKey
    ...
    <div className="min-h-screen bg-[var(--color-bg)]">
      ...
      <main className="mx-auto max-w-4xl space-y-8 px-4 py-8">
        ...
        <section>
          ...
          {periods.length === 0 ? (
            <p className="rounded-2xl bg-[var(--color-paper)] p-4 text-sm text-[var(--color-ink-soft)] ring-1 ring-[var(--color-line)]">
              Todavía no hay meses registrados para elegir.
            </p>
          ) : (
              ...
              >
                {monthsForYear.map((p) => (
                  <option key={p.month} value={p.month}>
                    {monthNameOnly(p.month)}
                  </option>
                ))}
              </select>
              <button
                disabled={!currentMonthSelection}
                onClick={() => goToMonth(currentMonthSelection)}
                className="rounded-lg bg-[var(--color-accent)] px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50"
              >
                Abrir mes
              </button>
              <p className="text-xs text-[var(--color-ink-soft)]">
                Solo se listan los meses que ya tienen registros.
              </p>
            </div>
          )}
        </section>
