...
/** Nombre del mes sin el año (ej. "Agosto"), para usar junto a un selector de año aparte. */
export function monthNameOnly(monthKey: string): string {
  const d = new Date(monthKey + "T00:00:00");
  const label = new Intl.DateTimeFormat("es-AR", { month: "long" }).format(d);
  return label.charAt(0).toUpperCase() + label.slice(1);
}
